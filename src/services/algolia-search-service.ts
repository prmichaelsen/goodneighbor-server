  ...
  async naturalLanguageToAlgoliaParams(query: string): Promise<AlgoliaSearchParams> {
    try {
      ...
      const relationshipPatterns = [
        ...
        { 
          relation: 'refs.hasTag', 
          verb: 'tagged with',
          keywords: ['tag', 'tags', 'tagged', 'tagged with', 'with tag', 'with the tag', 'hashtag', 'labeled', 'labeled as', 'categorized as']
        }
      ];
      ...
      const relationshipExamples = [
        ...
        `"posts tagged with #safety" → { "query": "", "filters": "type:post AND refs.hasTag:safety" }`
      ];
...
8. IMPORTANT: For queries about tags, use refs.hasTag instead of the query field
   - For example, "posts tagged with #safety" should map to { "query": "", "filters": "type:post AND refs.hasTag:safety" }
   - NOT { "query": "safety", "filters": "type:post" }
9. For queries about authors, creators, owners, etc., use the refs-based filters:
...
`;
      ...
      // Log the parameters generated by DeepSeek
      info(`Algolia parameters from DeepSeek for query "${query}":`, JSON.stringify(algoliaParams, null, 2));
      
      // Set default values for pagination if not provided
      if (algoliaParams.hitsPerPage === undefined) {
        algoliaParams.hitsPerPage = 20;
      }
      
      if (algoliaParams.page === undefined) {
        algoliaParams.page = 0;
      }
